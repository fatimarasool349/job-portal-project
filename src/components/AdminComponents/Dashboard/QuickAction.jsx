...
function QuickAction({ actions }) {
  ...
  // Get role from localStorage and normalize it
  const role = localStorage.getItem("role")?.trim().toLowerCase();

  if (!role) {
    // If role is missing, show a message instead of crashing
    return (
  ...
  // Filter actions safely
  const visibleActions = actions.filter(action => {
    if (!action.for) return false;

    // Ensure action.for is an array
    const allowedRoles = Array.isArray(action.for) ? action.for : [action.for];

    // Normalize all roles to lowercase and check if current role is included
